...
func (a *UUIDArray) Scan(value interface{}) error {
	//{23113aljdflafl,3k21313akfalfla}
	var str string
	...
	*a = make(UUIDArray, 0, len(parts))
	for _, s := range parts {
		s = strings.TrimSpace(strings.Trim(s, `"`)) // remove quotes and spaces
		if s == "" {
			continue
		}
		u, err := uuid.Parse(s)
...
// {"55ejfl50-41741914-4141-412313145719479194","55ejfl50-41741914-4141-412313145719479194"}
